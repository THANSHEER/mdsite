...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mdsite-ignore-'));
});
...
describe('loadIgnorePatterns', () => {
  ...
  it('reads patterns from .mdsiteignore (strips comments and blanks)', async () => {
    await fs.writeFile(
      path.join(tmpDir, '.mdsiteignore'),
      '# OS files\n.DS_Store\n\nnode_modules/\n',
    );
    const patterns = await loadIgnorePatterns(tmpDir);
  ...
  it('merges .mdsiteignore + .gitignore in that order', async () => {
    await fs.writeFile(path.join(tmpDir, '.mdsiteignore'), 'node_modules/\n');
