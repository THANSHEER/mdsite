...
async function buildFixtures(overrides: Record<string, unknown> = {}): Promise<string> {
  const out = await fs.mkdtemp(path.join(os.tmpdir(), 'mdsite-test-'));
...
describe('build (fixtures)', () => {
  ...
  it('wires the client runtime script into pages', async () => {
    const html = await read(out, 'getting-started/index.html');
    expect(html).toContain('mdsite.client.js');
  });
  ...
  it('renders mermaid fences as <pre class="mermaid"> and ships the chunk', async () => {
    ...
    expect(await exists(out, 'mdsite.mermaid.js')).toBe(true);
  });
...
describe('social / OG tags', () => {
  ...
  let out: string;
  beforeAll(async () => {
    dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mdsite-og-'));
    out = path.join(dir, 'out');
    await fs.writeFile(
      path.join(dir, 'mdsite.config.json'),
...
describe('comments (giscus)', () => {
  ...
  let out: string;
  beforeAll(async () => {
    dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mdsite-comments-'));
    out = path.join(dir, 'out');
    await fs.writeFile(
      path.join(dir, 'mdsite.config.json'),
      JSON.stringify({
...
describe('i18n', () => {
  ...
  let out: string;
  beforeAll(async () => {
    dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mdsite-i18n-'));
    out = path.join(dir, 'out');
    await fs.writeFile(
      path.join(dir, 'mdsite.config.json'),
...
describe('build with basePath', () => {
  ...
  beforeAll(async () => {
    ...
    dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mdsite-base-'));
    out = path.join(dir, 'out');
    await fs.writeFile(
      path.join(dir, 'mdsite.config.json'),
  ...
  it('prefixes asset, css, client and page hrefs with the base path', async () => {
    ...
    expect(html).toContain('src="/notes/mdsite.client.js"');
