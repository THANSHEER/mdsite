...
export interface BuildOptions {
  ...
  outDir?: string;
  /** Explicit path to mdsite.config.json. */
  configPath?: string;
...
export async function build(opts: BuildOptions = {}): Promise<BuildResult> {
  ...
  const { pages, assets } = await (async () => {
    ...
    if (Array.isArray(legacyIgnore) && legacyIgnore.length > 0) {
      console.warn(
        '\n⚠  build.ignore in mdsite.config.json is no longer supported.\n' +
        '   Move your patterns to .mdsiteignore instead.\n' +
        '   See: https://github.com/THANSHEER/mdsite#mdsiteignore\n',
      );
    }
  ...
  const ctx: RenderContext = {
    ...
    clientJsHref: withBase('/mdsite.client.js'),
  ...
  if (client) await writeOut(outDir, 'mdsite.client.js', client);

  if (config.features.mermaid && pages.some((p) => p.html.includes('class="mermaid"'))) {
    const mermaidJs = await getMermaidRuntime();
    if (mermaidJs) await writeOut(outDir, 'mdsite.mermaid.js', mermaidJs);
  }
