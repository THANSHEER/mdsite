...
// Default content of the .mdsiteignore file created by `mdsite init`.
const DEFAULT_MDSITEIGNORE = `# OS files
.DS_Store
Thumbs.db
desktop.ini
...
# mdsite config (not content)
mdsite.config.json
`;

export const CONFIG_FILENAME = 'mdsite.config.json';
...
export async function runConfigWizard(cwd: string): Promise<WizardResult> {
  p.intro('mdsite — set up your site');
...
/** Scaffold a mdsite.config.json and a sample content/ folder. */
export async function initSite(dir: string, opts: InitOptions = {}): Promise<void> {
  const root = path.resolve(process.cwd(), dir);
  await fs.mkdir(path.join(root, 'content'), { recursive: true });

  // Scaffold .mdsiteignore if it does not exist yet.
  const mdsiteignorePath = path.join(root, '.mdsiteignore');
  if (!(await fileExists(mdsiteignorePath))) {
    await fs.writeFile(mdsiteignorePath, DEFAULT_MDSITEIGNORE);
  }
...
This is your new mdsite. Edit \`content/index.md\` and add more \`.md\` files,
then run \`mdsite build\`.
...
`;
  ...
  console.log(`✓ Initialised mdsite in ${path.relative(process.cwd(), root) || '.'}`);
}
