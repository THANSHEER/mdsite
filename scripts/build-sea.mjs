// Builds a standalone `mdgarden` executable for the CURRENT OS/arch using Node's
...
//   MDGARDEN_SEA_NODE=/path/to/node-vXX/bin/node npm run build:binary
//
// Pipeline:
//   1. npm run build            -> dist/ (incl. the browser client runtime)
//   2. esbuild --binary         -> build/mdgarden.bundle.cjs (all deps inlined)
//   3. embed-assets             -> sea-config.json + build/katex-fonts.json
//   4. node --experimental-sea-config -> build/mdgarden.blob
...
const baseNode = process.env.MDGARDEN_SEA_NODE || process.execPath;
...
if (!supportsSea(baseNode)) {
  ...
  console.error(
    `\n✗ Base Node binary does not support SEA (no fuse sentinel):\n` +
      ...
      `    MDGARDEN_SEA_NODE=/path/to/node-vXX-${process.platform}-${process.arch}/bin/node \\\n` +
...
const outName = isWin ? 'mdgarden.exe' : 'mdgarden';
...
const postjectArgs = [
  ...
  outPath,
  'NODE_SEA_BLOB',
  path.join(buildDir, 'mdgarden.blob'),
  '--sentinel-fuse',
  FUSE,
];
