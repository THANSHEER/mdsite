// Rewrites packaging/homebrew/mdgarden.rb for a release: sets `version`, the
...
// mdgarden-<os>-<arch>.tar.gz assets. Missing assets are left unchanged.
// Version comes from package.json, or $MDGARDEN_RELEASE_VERSION.
...
const version = process.env.MDGARDEN_RELEASE_VERSION || pkg.version;
const relDir = process.argv[2] || path.join(root, 'build', 'release');
const formulaPath = path.join(root, 'packaging', 'homebrew', 'mdgarden.rb');

const assets = [
  'mdgarden-darwin-arm64.tar.gz',
  'mdgarden-darwin-x64.tar.gz',
  'mdgarden-linux-arm64.tar.gz',
  'mdgarden-linux-x64.tar.gz',
];
