...
> [!NOTE]
> This is a mirror of `CLAUDE.md`. Both files serve the same purpose: providing vital context to AI coding assistants.

This file provides crucial context for any AI assistant (like Claude, GitHub Copilot, Gemini, Cursor, etc.) working on the `mdsite` repository. Please read this before proposing architectural changes or adding new features.

## Project Goal
`mdsite` is a lightning-fast, zero-config static site generator that turns a folder of Markdown notes into a fully-featured digital garden (with backlinks, local/global interactive graphs, and search). 
...
2. **Do not introduce large frontend frameworks.**
   `mdsite` prides itself on shipping minimal JavaScript. Do not propose adding React, Vue, Svelte, or TailwindCSS. Use Vanilla JS and standard CSS.
3. **Do not break the Single Executable App (SEA) compatibility.**
   `mdsite` can be compiled into a standalone binary. Asset loading (like `mdsite.client.js` or KaTeX fonts) is handled via `src/parser/assets.ts` which uses `node:sea` to read assets when bundled. Do not use standard `fs.readFile` for core assets without utilizing the asset resolver.
4. **Do not hardcode versions.**
   The version number is defined exclusively in `package.json`. It is injected globally into the runtime as `MDSITE_VERSION` via `esbuild.config.mjs` using `define`. Use `export const VERSION = typeof MDSITE_VERSION !== 'undefined' ? MDSITE_VERSION : 'unknown';` as seen in `src/index.ts`.
