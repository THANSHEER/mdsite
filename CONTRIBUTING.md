# Contributing to mdsite

This document outlines how to build, run, and test `mdsite` locally.
...
`mdsite` uses [Vitest](https://vitest.dev/) for unit and integration testing.
...
To bundle `mdsite` as a single executable binary for your current OS and architecture:
```bash
npm run build:binary
```
The output executable will be placed in `build/mdsite` (or `build/mdsite.exe` on Windows).
...
> If your local Node was installed via Homebrew, download an official binary from [nodejs.org/dist](https://nodejs.org/dist) and set the `MDSITE_SEA_NODE` environment variable:
> ```bash
> MDSITE_SEA_NODE=/path/to/official/node/bin/node npm run build:binary
> ```
